...
function MsgConfirmacao(Msg, Title: String): Boolean;
begin
  Result := UnitFuncoes.MsgCompleta(Msg, Title, 'Sim', 'Não', 1, False, False);
end;

function MsgFixa(tipoMsg: TTipoMsg; msg : String = ''): Boolean;
begin
  if tipoMsg = tMsgErroPK then Result := MsgErro('Esse Registro é Protegido e não pode ser Excluído!')
  else if tipoMsg = tMsgDelBtn then Result := MsgConfirmacao('Deseja Excluir o Registro?', 'Excluir Registro')
  else if tipoMsg = tMsgCloseFrm then Result := MsgConfirmacao('Deseja Fechar o formulário?', 'Fechar Formulário')
  else if tipoMsg = tMsgCancBtn then Result := MsgConfirmacao('Deseja Cancelar as alterações?', 'Cancelar Alterações')
  else if tipoMsg = tMsgCampoBranco then Result := MsgConfirmacao('O campo ' + msg + ' Está em branco. ' + #13 +
                'Deseja continuar mesmo assim?', 'Campo em Branco')
  else if tipoMsg = tMsgCampObrig then Result := MsgAviso('É obrigatório preencher o campo ' + msg)
end;
